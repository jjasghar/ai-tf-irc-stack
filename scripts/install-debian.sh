...
# Debian-specific IRC Stack Installation Script
# Used by: IBM Cloud (ibm-debian-12-0-minimal-amd64-1)
...
echo "======================================"
echo "  IRC Stack Installation - Debian"
echo "======================================"
echo "Hostname: $HOSTNAME"
echo "Admin Email: $ADMIN_EMAIL"
echo "Ergo Network: $ERGO_NETWORK"
echo "Ergo MOTD: $ERGO_MOTD"
echo "OS: Debian 12 (detected automatically)"
echo "======================================"
...
# Ensure ufw is installed (in case previous installation failed)
apt-get install -y ufw || echo "Warning: ufw installation failed, skipping firewall configuration"

# Check if ufw is available before configuring
if command -v ufw &>/dev/null; then
    echo "Starting and enabling ufw..."
    systemctl start ufw || echo "Warning: ufw service start failed"
    systemctl enable ufw || echo "Warning: ufw service enable failed"
    ufw --force enable
    ...
    echo "⚠️  ufw not available, skipping firewall configuration (ports will be open)"
fi
...
# Configure Caddy with HTTPS directly (Debian-specific config)
echo "Configuring Caddy (HTTPS with automatic SSL)..."
cp /tmp/configs/Caddyfile.debian /etc/caddy/Caddyfile
